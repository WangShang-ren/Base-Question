...
通信绑定 --bind-to none（无 CPU 核心绑定）
网络层 PML: ob1，BTL: tcp,self（TCP/IP 回环/虚拟网卡通信）
问题规模 128 \times 128 \times 128（全局网格）
时间步数 20 steps
处理器拓扑 2 \times 2 \times 2（npx=2, npy=2, npz=2）

3. 复现过程与关键步骤

3.1 初始遇到的问题：断言失败（Assertion Failed）

初次尝试运行时，由于参数传递错误，导致程序崩溃。
...
· 64 64 64：每个进程分配的局部网格大小为 64 \times 64 \times 64（总全局 128^3）。
· 20：执行 20 个时间步长。
...
由于使用的是 Reference Kernel（参考内核），未链接 MKL/OpenBLAS 等高性能库，性能较低，主要用于功能验证。
...
1. 复现成功：HPCG 3.1 在 WSL2 环境下，使用 8 进程 MPI 并行计算成功运行并输出有效结果。
2. 问题解决：解决了初期因参数错误导致的 Assertion 'nxf%2==0' failed 崩溃问题，确认了 128^3 规模在 8 进程下的正确分解方式。
3. 结果性质：获得的是 Valid（有效）的基准测试结果，而非 Optimized（优化）的高性能结果。
...
使用 mpirun.openmpi 运行 HPCG（64×64×64 网格）：
